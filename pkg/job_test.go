...
func TestLogRotationPeriodParsing(t *testing.T) {
	tests := []struct {
		name        string
		durationStr string
		shouldError bool
		...
		},
		{
			...
			durationStr: "invalid",
			shouldError: true,
			expectedErr: "invalid log_rotation_period",
		},
		{
			name:        "zero duration",
		...
		{
			name:        "old format not supported - complex",
			durationStr: "1h30m",
			shouldError: true,
			expectedErr: "invalid log_rotation_period",
		},
...
func TestLogRotationIntegration(t *testing.T) {
	...
	// Create a job with log rotation
	j := &JobSpec{
		Name:                "test-rotation",
		Command:             []string{"echo", "test"},
		logRotationDuration: 1 * time.Hour, // Rotate logs older than 1 hour
		cfg:                 cfg,
...
func TestLogRotationNoDuration(t *testing.T) {
	// Test that rotation doesn't run when no duration is set
	db, err := OpenDB(":memory:")
	if err != nil {
		t.Fatal(err)
	}
	...
	// Run log rotation - should do nothing
	j.rotateLogs()
...
func TestLogRotationNoDB(t *testing.T) {
	// Test that rotation doesn't crash when no DB connection
	j := &JobSpec{
		Name:                "test-no-db",
		Command:             []string{"echo", "test"},
		logRotationDuration: 1 * time.Hour,
